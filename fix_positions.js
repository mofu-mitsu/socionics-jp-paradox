...
const splitStartIndex = content.indexOf('// ==========================================================================');
const firstTarget = content.indexOf('q_ili_lii_lsi_split_1: {');
if (firstTarget !== -1) {
    const linesBefore = content.substring(0, firstTarget).split('\n');
    let startIdx = firstTarget;
    // Walk back to find the comment block
    for(let i=linesBefore.length-1; i>=0; i--) {
        if(linesBefore[i].includes('// ==========================================================================')) {
            startIdx = content.indexOf(linesBefore[i]);
    ...
    if (endIdx !== -1 && startIdx !== -1) {
        ...
        console.log("Successfully replaced questions with 8 full positions");
    } else {
        console.error("Could not find start or end index");
    }
} else {
    console.error("Could not find q_ili_lii_lsi_split_1");
}
