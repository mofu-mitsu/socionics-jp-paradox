...
const qGameChappyStart = content.indexOf('q_game_chappy: {');
const q8Start = content.indexOf('  q8: {', qGameChappyStart);
...
const before = content.slice(0, qGameChappyStart);
const after = content.slice(content.lastIndexOf('  // --------------------------------------------------------------------------\n  // q8: 規範', q8Start));

// Wait, the after might not match if it has carriage returns. Let's just slice from q8Start and prepend the missing comment.

const finalAfter = '  // --------------------------------------------------------------------------\n  // q8: 規範と義務へのスタンス（INFP・ISFP的リアリティ反映版）\n  // --------------------------------------------------------------------------\n' + content.slice(q8Start);

fs.writeFileSync('lib/questions.ts', before + cleanChappy + finalAfter, 'utf8');
console.log('Done!');
